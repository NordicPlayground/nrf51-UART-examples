...
#define DEVICE_NAME                     "UART_master"                               /**< Name of device. Will be included in the advertising data. */
...
#define BLE_TO_UART_ACTIVITY_LED_PIN_NO 10
#define UART_TO_BLE_ACTIVITY_LED_PIN_NO 11
...
static uint8_t * p_data_global;
static uint8_t data_index = 0;
static uint8_t data_countdown = 0;
...
void nus_data_handler(ble_nus_t * p_nus, uint8_t * p_data, uint16_t length)
{
		...
		p_data_global = p_data;
		data_countdown = length - 1;
		data_index = 0;
	
		//Sending multiple bytes on the UART. Can only be done with FIFO buffer.
    for (int i = 0; i < length; i++)
    {
				error_code = app_uart_put(p_data[i]);
    }
		app_uart_put('\n');
		
		//Enable the UART in order to transmit the data in the TX FIFO buffer.
        ...
        case APP_UART_DATA:	
            ...
            break;
				...
				case APP_UART_TX_EMPTY:
						//Data has been successfully transmitted on the UART and the TX FIFO buffer is empty.
						//Disable the UART to save power. 
						//This will also disable the UART on the connecting device if the flow 
						//control option APP_UART_FLOW_CONTROL_LOW_POWER is set.
